...
targetVec = cmds.getAttr('target_vector.translate')[0]
m = Matrix('new_basis')
#row by row
accumulator = []
for row in m.matrix[:3]:
    accuI = 0.0
    for i in range(len(targetVec)):
        accuI += targetVec[i] * row[i]
    accumulator.append(accuI)
print(accumulator)


cmds.setAttr('reference_vector.translateX', accumulator[0])
cmds.setAttr('reference_vector.translateY', accumulator[1])
cmds.setAttr('reference_vector.translateZ', accumulator[2])


oneD = distance([0.0, 0.0, 0.0], getPositions('target_vector'))
twoD = distance(getPositions('new_basis'), getPositions('reference_vector'))
